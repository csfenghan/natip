...
void ClientData::printClientData() {
        printf("data:\n\tname:%s\n\taddr:%s\n\tport::%d\n\tpid:%"
               "d\n\tinfo:%s\n\n",
               getName().data(), getAddr().data(), getPort(), getPid(),
               getInfo().data());
}

// 设置所有的数据
void ClientData::setAllData(std::string name, std::string addr, int port,
                            int pid, std::string info) {
        setName(name);
        setAddr(addr);
        setPort(port);
        setPid(pid);
...
static bool manageMySQL(ServiceType service, ClientData &data,
                        std::string name = std::string(), int pid = -1) {
        bool result = true;
        ...
        try {
                ...
                case ADD_CLIENT: // 添加
                        snprintf(sql_str, sizeof(sql_str),
                                 ...
                                 "VALUES('%s','%s',%d,%d,'%s')",
                                 data.getName().c_str(), data.getAddr().c_str(),
                                 data.getPort(), data.getPid(),
                                 data.getInfo().c_str());
                        stmt->execute(sql_str);
                        break;
                case DEL_CLIENT: // 删除
                        if (pid != -1)
                                snprintf(sql_str, sizeof(sql_str),
                                         "DELETE FROM client_data WHERE pid=%d",
                                         pid);
                        else
                                snprintf(
                                    sql_str, sizeof(sql_str),
                                    "DELETE FROM client_data WHERE name='%s'",
                                    name.c_str());
                        stmt->execute(sql_str);
                ...
                case FIND_CLIENT: // 搜索
                        if (pid != -1)
                                snprintf(
                                    sql_str, sizeof(sql_str),
                                    "SELECT * FROM client_data WHERE pid=%d",
                                    pid);
                        else
                                snprintf(
                                    sql_str, sizeof(sql_str),
                                    "SELECT * FROM client_data WHERE name='%s'",
                                    name.data());
                        ...
                        if (res->next()) {
                                data.setAllData(
                                    res->getString("name"),
                                    res->getString("addr"), res->getInt("port"),
